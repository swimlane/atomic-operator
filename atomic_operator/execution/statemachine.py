...
class InnvocationState(State, Base):
    ...
    def __handle_windows_errors(self, stream):
        ...
        for item in stream.error:
            return_list.append({"type": "error", "value": str(item)})
        for item in stream.debug:
            return_list.append({"type": "debug", "value": str(item)})
        for item in stream.information:
            return_list.append({"type": "information", "value": str(item)})
        for item in stream.verbose:
            return_list.append({"type": "verbose", "value": str(item)})
        for item in stream.warning:
            return_list.append({"type": "warning", "value": str(item)})
        return return_list

    def __create_win_client(self, hostinfo):
        self.__win_client = Client(
            hostinfo.hostname, username=hostinfo.username, password=hostinfo.password, ssl=hostinfo.verify_ssl
        )
    ...
    def __invoke_cmd(self, command, input_arguments=None, elevation_required=False):
        ...
        command = self._replace_command_string(
            command, path="c:/temp", input_arguments=input_arguments, executor="command_prompt"
        )
        ...
        if stderr:
            self.__logger.error(
                "{host} responded with the following message(s): {message}".format(
                    host=self.hostinfo.hostname, message=stderr
                )
            )
        return ParseResultsState(command=command, return_code=rc, output=stdout, error=stderr)
    ...
    def __invoke_powershell(self, command, input_arguments=None, elevation_required=False):
        ...
        Copier(windows_client=self.__win_client, elevation_required=elevation_required).copy(
            input_arguments=input_arguments
        )
        command = self._replace_command_string(
            command, path="c:/temp", input_arguments=input_arguments, executor="powershell"
        )
        # TODO: NEED TO ADD LOGIC TO TRANSFER FILES TO WINDOWS SYSTEMS USING POWERSHELL
        if elevation_required:
        ...
        if had_errors:
            self.__logger.error(
                "{host} responded with the following message(s): {message}".format(
                    host=self.hostinfo.hostname, message=self.__handle_windows_errors(streams)
                )
            )
        return ParseResultsState(
            command=command, return_code=had_errors, output=output, error=self.__handle_windows_errors(streams)
        )
    ...
    def __invoke_ssh(self, command, input_arguments=None, elevation_required=False):
        ...
        try:
            ...
            ssh_stdin, ssh_stdout, ssh_stderr = ssh.exec_command(shlex.quote(command))
        except Exception as e:
            self.__logger.debug(f"Error executing command using shlex: {e}")
        ...
        ssh_stdin.flush()
        ssh.close()
        return ParseResultsState(command=command, return_code=return_code, output=out, error=err)
    ...
    def invoke(self, hostinfo, command_type, command, input_arguments=None, elevation_required=False):
        ...
        result = None
        if command_type == "powershell":
            result = self.__invoke_powershell(
                command, input_arguments=input_arguments, elevation_required=elevation_required
            )
        elif command_type == "cmd":
...
class ParseResultsState(State, Runner):
    ...
    def __init__(self, command=None, return_code=None, output=None, error=None):
        self.result = {}
        self.print_process_output(command=command, return_code=return_code, output=output, errors=error)
        if output:
